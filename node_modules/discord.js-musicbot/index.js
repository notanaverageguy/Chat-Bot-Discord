    ...
    try {
        ...
        class Music {
            constructor(client, options) {
                ...
                this.lyrics = {
                    enabled: (options.lyrics == undefined ? true : (options.lyrics && typeof options.lyrics.enabled !== 'undefined' ? options.lyrics && options.lyrics.enabled : true)),
                    run: "lyricsFunction",
                    alt: (options && options.lyrics && options.lyrics.alt) || [],
                    help: (options && options.lyrics && options.lyrics.help) || "Fetches lyrics for the current song or specified song name.",
                    name: (options && options.lyrics && options.lyrics.name) || "lyrics",
                    usage: (options && options.lyrics && options.lyrics.usage) || null,
                    exclude: Boolean((options && options.lyrics && options.lyrics.exclude)),
                    masked: "lyrics"
                };
        ...
        musicbot.lyricsFunction = (msg, suffix, args, ignore) => {
            const baseURL = `https://api.genius.com/search?access_token=4DK4ChKlTuMLmZVmltW9MSX-AmBg1g70-kVbRRalCUCFrADcEkoHltSFTSSUdlR8`;
            ...
            const searchLyrics = url => {
                return Promise.resolve(axios.get(url, { "Authorization": `Bearer ${process.env.GENIUS}` })
            ...
            if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', 'Nothing is playing!'));
            ...
            var songTitle = queue.last.title.replace(/\\/g, '\\\\')
                ...
                .replace(/`/g, '\\`');
            if (!suffix) {
                msg.channel.send(musicbot.note('note', `Usage: ${exports.lyrics.usage}`))
            }

            if (suffix === "np") {
                searchLyrics(`${baseURL}&q=${encodeURIComponent(songTitle)}`)
                    .then(songData => {
                        const embed = new Discord.RichEmbed()
                            .setColor(0x00AE86)
                    ...
                    .catch(err => {
                        msg.channel.send(`No lyrics found for: ${songTitle} 🙁`, { code: "asciidoc" });
                        console.warn(err);
                    });
                ...
                searchLyrics(`${baseURL}&q=${encodeURIComponent(suffix)}`)
                    .then(songData => {
                        const embed = new Discord.RichEmbed()
                            .setColor(0x00AE86)
                            .setTitle(`Lyrics for: ${songData[0]}`)
                    ...
                    .catch(err => {
                        msg.channel.send(`No lyrics found for: ${query} 🙁`, { code: "asciidoc" });
                        console.warn(err);
                    });
            }
        };
        ...
        musicbot.loadCommands = async () => {
            try {
                ...
                await musicbot.loadCommand(musicbot.lyrics);
